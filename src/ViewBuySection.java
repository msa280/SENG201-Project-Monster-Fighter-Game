...
public class ViewBuySection {
	...
	{
		Monster monster = this.buyShop.getMonstersForSale().get(buttonIndex-1);
	...
	public void buyMonster(int buttonIndex, JTextPane updateArea, JTextField goldDisplay)
	{
		...
		for (Monster monster: this.buyShop.getMonstersForSale())
		{
			if (i == buttonIndex)
			{
	...
	public void buyItem(int buttonIndex, JTextPane updateArea, JTextField goldDisplay)
	{
		...
		for (Item item: this.buyShop.getItemsForSale())
		{
			if (i == buttonIndex)
			{
		...
		System.out.print(this.player.getPlayerInventory());
	}
	...
	{
		String update = this.buyShop.getItemsForSale().get(buttonIndex-1).getItemEffect();
	...
	public ViewBuySection(Player player) {
		this.player = player;
		this.buyShop = this.player.getShop();
		this.audioPlayer.playSound("ShopSection.wav");
		initialize();
	}
	...
	private void initialize() {
		...
		while (i < this.buyShop.getMonstersForSale().size())
		{
			Monster monster = this.buyShop.getMonstersForSale().get(i);
			Item item = this.buyShop.getItemsForSale().get(i);
			mimages[i].setIcon(new ImageIcon(chooseMonsterImage(monster)));
			iimages[i].setIcon(new ImageIcon(chooseItemImage(item)));
